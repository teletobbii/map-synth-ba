...
# attaches the three maps of the final data sample next to each other 
...
def batch(src, des):
    ...
    for i in range(int(len(files) / 3)):
        ...
        tmap = cv.imread(src + str(i+1) + "_t.png", cv.IMREAD_COLOR)
        ...
        combined = np.concatenate((tmap, bmap), axis=1)
        combined2 = np.concatenate((combined, hmapc), axis=1)
        cv.imwrite(des + str(i+1) + ".png", combined2)
...
def main():
    ...
    folderName = "C:/Users/tobia/BA/map-synth-ba/dataset/new/notReady/bmt_maps_with_underworlds/"
    destinationFolder = "C:/Users/tobia/BA/map-synth-ba/dataset/new/ready/t2bh_data_underworlds/"
