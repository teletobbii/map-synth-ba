...
def colourFix():
    ...
    generated_biomes = cv.imread(r"C:\Users\tobia\OneDrive\Desktop\show\10_11\toShow\both/" + "1_biomes_gen.png", cv.IMREAD_COLOR)
    ...
    allColours = {
        ...
        8: np.array([0, 0, 255]),       #lava
        9: np.array([0, 0, 35]),   #obsidian
        10: np.array([90, 145, 90]),   #swamp
        11: np.array([30, 160, 210]),      #desert
        12: np.array([90, 80, 130]),       #wasteland
        13: np.array([128, 128, 128]),    #rocks
        14: np.array([210, 170, 135])    #ice
    }
    ...
    for i in range(rows):
        for j in range(columns):
            if (water[i][j] == 255):
                fixed[i][j] = allColours[1]
            elif (desert[i][j] == 255):
                fixed[i][j] = allColours[11]
            elif (sand[i][j] == 255):
                fixed[i][j] = allColours[2]
            elif (mountains[i][j] == 255):
                fixed[i][j] = allColours[6]
            elif (peak[i][j] == 255):
                fixed[i][j] = allColours[7]
            elif (lava[i][j] == 255):
                fixed[i][j] = allColours[8]
            elif (obsidian[i][j] == 255):
                fixed[i][j] = allColours[9]
            elif (swamp[i][j] == 255):
                fixed[i][j] = allColours[10]
            elif (wasteland[i][j] == 255):
                fixed[i][j] = allColours[12]
            elif (rocks[i][j] == 255):
                fixed[i][j] = allColours[13]
            elif (peak[i][j] == 255):
                fixed[i][j] = allColours[14]
            elif (forest[i][j] == 255):
                fixed[i][j] = allColours[3]
            elif (grass[i][j] == 255):
                fixed[i][j] = allColours[4]
            elif (steppe[i][j] == 255):
                fixed[i][j] = allColours[5]
            ...
            for j in np.random.permutation(126):
                ...
                if (fixed[i][j] == 0).all():
                    ...
                    for k in range(len(arr)):
                        if ((arr[k] == allColours[1]).all()):
                            countArray[k] = 1
                        elif ((arr[k] == allColours[2]).all()):
                            countArray[k] = 2
                        elif ((arr[k] == allColours[3]).all()):
                            countArray[k] = 3
                        elif ((arr[k] == allColours[4]).all()):
                            countArray[k] = 4
                        elif ((arr[k] == allColours[5]).all()):
                            countArray[k] = 5
                        elif ((arr[k] == allColours[6]).all()):
                            countArray[k] = 6
                        elif ((arr[k] == allColours[7]).all()):
                            countArray[k] = 7
                        elif ((arr[k] == allColours[8]).all()):
                            countArray[k] = 1
                        elif ((arr[k] == allColours[9]).all()):
                            countArray[k] = 2
                        elif ((arr[k] == allColours[10]).all()):
                            countArray[k] = 3
                        elif ((arr[k] == allColours[11]).all()):
                            countArray[k] = 4
                        elif ((arr[k] == allColours[12]).all()):
                            countArray[k] = 5
                        elif ((arr[k] == allColours[13]).all()):
                            countArray[k] = 6
                        elif ((arr[k] == allColours[14]).all()):
                            countArray[k] = 7
                    if not (np.count_nonzero(countArray == 0) == 8):
                        toFill = mode(countArray[np.nonzero(countArray)])
                        fixed[i][j] = allColours[toFill]
                    else:
                        thereAreStillBlackSpots = True
    ...
    cv.imwrite(r"C:\Users\tobia\OneDrive\Desktop\show\10_11\toShow\both/" + "1_biomes_fixed.png", fixed)
